...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        if (linkError) {
            console.error('[dr7trust-resend-verification] Generate link error:', linkError.message)
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color: #f9fafb;">
    <tr>
      <td align="center" style="padding: 40px 20px;">
        ...
        if (emailError) {
            console.error('[dr7trust-resend-verification] Resend error:', emailError)
            return { statusCode: 500, body: JSON.stringify({ error: 'Errore nell\'invio dell\'email' }) }
        }

        console.log(`[dr7trust-resend-verification] Verification email resent to ${email}`)
        ...
        console.error('[dr7trust-resend-verification] Error:', error)
        return {
            statusCode: 500,
